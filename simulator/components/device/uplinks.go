...
func (d *Device) CreateUplinkWithIDs(seqIds []int) [][]byte {
	...
	if d.Info.Configuration.SupportedFragment { //frammentazione
		...
		if len(d.Info.Status.DataUplink.FOpts) > 0 {
			DataPayload = up.FragmentationWithIds(seqIds, d.Info.DeviceType, d.Info.DataType, d.Info.AxisId, n, payload)
		} else {
			DataPayload = up.FragmentationWithIds(seqIds, d.Info.DeviceType, d.Info.DataType, d.Info.AxisId, m, payload)
		}
